...
import { Search, Menu, X, Gamepad2, Home, Mail, Star, List, Calendar, Grid } from 'lucide-react';
      ...
      >
        <div className="p-6 pt-20">
          ...
          <form onSubmit={handleSearch} className="mb-6">
            <div className="flex w-full">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search..."
                className="flex-1 px-3 py-2 rounded-l-full bg-white text-gray-800 focus:outline-none text-sm w-full"
              />
              <button
                type="submit"
                className="flex items-center justify-center px-3 py-2 bg-red-600 rounded-r-full hover:bg-red-700 transition-colors"
              >
                <Search size={16} />
              </button>
            </div>
          ...
          <nav className="space-y-2">
            ...
            <Link href="/contact" onClick={toggleSidebar} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-red-600 transition-all border-l-2 border-transparent hover:border-red-600 hover:pl-5">
              <Mail size={20} /> Contact us
            </Link>
            <Link href="/request" onClick={toggleSidebar} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-red-600 transition-all border-l-2 border-transparent hover:border-red-600 hover:pl-5">
            ...
            <Link href="/top-rated" onClick={toggleSidebar} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-red-600 transition-all border-l-2 border-transparent hover:border-red-600 hover:pl-5">
              <Star size={20} /> Top Rated
            </Link>
            <Link href="/games" onClick={toggleSidebar} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-red-600 transition-all border-l-2 border-transparent hover:border-red-600 hover:pl-5">
              <List size={20} /> All Games
            </Link>
            <Link href="/latest" onClick={toggleSidebar} className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 hover:text-red-600 transition-all border-l-2 border-transparent hover:border-red-600 hover:pl-5">
